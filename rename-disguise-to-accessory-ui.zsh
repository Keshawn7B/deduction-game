...
# Add a type alias so future code can say Accessory without breaking existing Disguise fields.
path = Path("src/types/card.ts")
text = path.read_text()

if "export type Accessory = Disguise" not in text:
    text = text.replace(
    ...
    # Insert alias after the Disguise union block.
    marker = "export type Location ="
    if marker in text:
        text = text.replace(marker, "export type Accessory = Disguise\n\n" + marker, 1)
    else:
        print("Could not find Location marker in src/types/card.ts; skipping Accessory alias.")
...
files = [
    Path("src/components/game/CardView.tsx"),
    Path("src/components/game/LayeredCardArt.tsx"),
    Path("src/components/game/CardArt.tsx"),
    Path("src/pages/GuessPage.tsx"),
    Path("public/assets/cards/README.md"),
]
...
for path in files:
    ...
    replacements = {
        ...
        "disguise": "accessory",
        "dressed as": "wearing",
        "Disguise overlay": "Accessory overlay",
        "Disguise/accessory": "Accessory",
    }

    # Do not rename actual Card fields or type names inside TS imports/logic blindly.
    if path.suffix in {".tsx", ".ts"}:
        text = text.replace(">Disguise<", ">Accessory<")
        text = text.replace("Disguise</span>", "Accessory</span>")
        text = text.replace("Disguise</p>", "Accessory</p>")
        text = text.replace("Dressed as", "Accessory")
        text = text.replace("dressed as", "wearing")
        text = text.replace("disguise/accessory", "accessory")
        text = text.replace("Disguise</p>", "Accessory</p>")
        text = text.replace("Disguise", "Accessory")
        # Restore code identifiers/types that must remain stable.
        text = text.replace("type { Animal, Card, Accessory, Location }", "type { Animal, Card, Disguise, Location }")
        text = text.replace("Record<Accessory,", "Record<Disguise,")
        text = text.replace("accessoryIcons[card.accessory]", "accessoryIcons[card.disguise]")
        text = text.replace("card.accessory", "card.disguise")
        text = text.replace("setAccessory", "setDisguise")
        text = text.replace("const [accessory,", "const [disguise,")
        text = text.replace("value={accessory}", "value={disguise}")
        text = text.replace("accessory,", "disguise,")
        text = text.replace("as Accessory", "as Disguise")
        text = text.replace("accessory={card.disguise}", "disguise={card.disguise}")
        text = text.replace("AccessoryLayer", "DisguiseLayer")
        text = text.replace("function DisguiseLayer({ accessory }", "function DisguiseLayer({ disguise }")
        text = text.replace("{ accessory }: { accessory: Disguise }", "{ disguise }: { disguise: Disguise }")
        text = text.replace("switch (accessory)", "switch (disguise)")
        text = text.replace("accessoryAssets[card.accessory]", "accessoryAssets[card.disguise]")
    else:
...
# Clean up likely bad replacements in cardAssets.ts explicitly.
python3 - <<'PY'
from pathlib import Path
...
if path.exists():
    text = path.read_text()
    text = text.replace("Disguise accessory", "Accessory")
    text = text.replace("Pirate accessory", "Pirate accessory")
    text = text.replace("Astronaut accessory", "Astronaut accessory")
    text = text.replace("Wizard accessory", "Wizard accessory")
    text = text.replace("Detective accessory", "Detective accessory")
    text = text.replace("Knight accessory", "Knight accessory")
    text = text.replace("Chef accessory", "Chef accessory")
    text = text.replace("Robot accessory", "Robot accessory")
    text = text.replace("Ninja accessory", "Ninja accessory")
    path.write_text(text)
PY

# Rewrite GuessPage cleanly to avoid accidental identifier changes.
    ...
    <section className="mx-auto min-h-screen max-w-6xl px-4 py-8">
      ...
      <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900 p-4">
        ...
        <p className="mt-1 text-sm text-slate-400">
          A correct animal, accessory, and location wins immediately. A wrong
...
echo ""
echo "Accessory wording patch complete."
