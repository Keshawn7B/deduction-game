...
path = Path("src/types/room.ts")
text = path.read_text()

if "wrongGuesses?: number" not in text:
    text = text.replace(
        "  joinedAt: unknown\n}",
        "  joinedAt: unknown\n  wrongGuesses?: number\n  eliminated?: boolean\n}",
    )
...
# New players should have public status fields.
if "wrongGuesses: 0," not in text.split("export async function startGame")[0]:
    text = text.replace(
        "      joinedAt: serverTimestamp(),\n",
        "      joinedAt: serverTimestamp(),\n      wrongGuesses: 0,\n      eliminated: false,\n",
    )

# When a game starts, mirror each private player state to the public player doc.
start_marker = """    batch.set(
...
"""
if start_marker in text and "players', playerState.playerId), {\n      wrongGuesses: 0" not in text:
    text = text.replace(start_marker, start_add)

# Wrong guesses should update the public player doc.
guess_marker = """  batch.set(playerStateRef, nextPlayerState)

  if (activePlayersAfterGuess.length === 1) {
"""
guess_add = """  batch.set(playerStateRef, nextPlayerState)
  ...
  if (activePlayersAfterGuess.length === 1) {
"""
if guess_marker in text and "wrongGuesses: penalty.wrongGuesses" not in text:
    text = text.replace(guess_marker, guess_add)

# Leaving during a game should update the public player doc.
leave_marker = """  batch.set(
...
leave_add = """  batch.set(
  ...
  if (room.hostId === params.playerId && remainingPlayers.length > 0) {
"""
if leave_marker in text and "players', params.playerId), {\n    eliminated: true" not in text:
    text = text.replace(leave_marker, leave_add)

# Rematch reset should clear public status.
rematch_marker = """    batch.update(doc(db, 'rooms', roomCode, 'players', player.id), {
      ready: player.id === room.hostId,
      isHost: player.id === room.hostId,
    })
"""
rematch_add = """    batch.update(doc(db, 'rooms', roomCode, 'players', player.id), {
      ready: player.id === room.hostId,
      isHost: player.id === room.hostId,
      wrongGuesses: 0,
      eliminated: false,
...
"""
if rematch_marker in text:
    text = text.replace(rematch_marker, rematch_add)
...
echo ""
echo "Player status panel patch complete."
